...
for EMD in Best_FVL:
    ...
    FE_time, FE_vol, FE_flow = sp.get_FE_signal(
        start_type='thresh_PEF', thresh_percent_begin=1
    )
    
    #%% AreaFE % predicted
    aFE = spirolib.spiro_features_extraction.areaFE(
        FE_vol, FE_flow, sex, age, height
    )
    ...
    #%% Angle of collapse
    ac = spirolib.spiro_features_extraction.angle_of_collapse(
        FE_vol, FE_flow
    )
    AC = ac.calc_AC(plotModel=True, plotProcess=True)
    
    #%% Deflating balloon model
    db = spirolib.spiro_features_extraction.deflating_baloon(
        FE_time, FE_vol, FE_flow
    )
