...
* `scale2`                  : Numeric factor to scale the input time vector (e.g., to convert milliseconds to seconds). Multiply `time` by this factor before processing.
...
* `check_acceptability_of_spirogram(min_FE_time=6, thresh_percent_end=0.5)`  
  Evaluates acceptability of the spirometry signal. Can also reject signals if BEV criteria are not met.
