...
* `scale2`: Numeric factor to multiply the `time` values before internal processing (e.g., use `1` if your `time` is already in seconds, `1000` if in milliseconds)
...
```python
# Scale factor = 1 if time is in seconds
