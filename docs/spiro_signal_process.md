...
* `time`: Time array of the spirometry manoeuvre (list or 1D array, in original units before scaling)
...
* `scale2`: Numeric scale factor to multiply the input `time` array so that internal `time` is in seconds (e.g., use `1` if input is already in seconds, or `0.001` if in milliseconds)
