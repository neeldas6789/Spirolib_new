...
```python
sp = spiro_signal_process(time, volume, flow, patientID, trialID, flag_given_signal_is_FE, scale5)
```
...
* `scale5`: Numerical factor to scale the input `time` array (e.g., 1 for seconds, 1000 for milliseconds)
...
```python
sp = spiro_signal_process(time, volume, flow, patientID='P1', trialID='T1', flag_given_signal_is_FE=False, scale5=1)
