...
* `time`: Time array of the spirometry manoeuvre (list or 1D array, before scaling)
...
* `scale1`: Multiplicative scale applied to the `time` input (e.g., to convert units to seconds)
* `scale2`, `scale3`, `scale4`: Additional scale parameters reserved for future extensions (currently unused)
