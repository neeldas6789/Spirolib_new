...
* `scale`: Scaling factor applied to the `time` values
...
* `get_FE_start_end(...)`
  * Determines the indices for the start and end of forced expiration
...
* `get_FE_signal(start_type=None, thresh_percent_begin=2, thresh_percent_end=0.25, plot=False)`
  * Returns FE segment (time, volume, flow) and optionally plots it. When `start_type` is `"BEV"` or `"thresh_PEF"`, the output signals are padded so flow and volume start/end at zero.
...
* `check_acceptability_of_spirogram(min_FE_time=6, thresh_percent_end=0.5)`
  * Evaluates acceptability of the spirometry signal; rejects if BEV criteria are not met or signal artefacts occur
...
* `finalize_signal(sex=None, age=None, height=None)`
  * Finalizes signal after processing and calculates all flow/volume metrics and predicted values. If not already set, `index1` and `index2` (start/end of FE) are determined during this step.
