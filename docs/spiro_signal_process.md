...
* `scale2`: Scaling factor to apply to time values (e.g., to convert input units to seconds)
...
* `shift_TLC_to_orgin()`
  * Shifts the volume signal so that the total lung capacity (TLC) at the start of forced expiration becomes zero
...
* Use `shift_TLC_to_orgin()` to reset the volume baseline to the point of total lung capacity
...
```python
# Instantiate the class (e.g., scale2=1 if time is already in seconds)
