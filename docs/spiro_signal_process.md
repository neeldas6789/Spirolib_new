...
* `scale1`: Multiplicative factor to convert input `time` to seconds (e.g., `1e-3` if input time is in milliseconds)
...
```python
# If input time is already in seconds, set scale1=1
