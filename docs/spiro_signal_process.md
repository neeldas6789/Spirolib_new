...
sp = spiro_signal_process(
    ...
    scale  # scaling factor applied to time input (e.g., convert raw units to seconds)
)
```
...
* `scale`: Numeric factor multiplied with `time` to standardize units (e.g., to seconds)
...
* `manual_trim(begin_time=0, end_time=None)`
  * Trims the signal between the given time bounds and returns `(time, volume, flow)` arrays
...
* `get_FE_signal(start_type=None, thresh_percent_begin=2, thresh_percent_end=0.25, plot=False)`
  * Returns FE segment `(time, volume, flow)` and optionally plots it. For `BEV` or `thresh_PEF` `start_type`, the output signals are padded so flow and volume start/end at zero.
...
* It is important to apply `correct_data_positioning()` before unit standardization and acceptability checks.
