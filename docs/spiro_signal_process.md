...
```python
# Note: `scale2` applies a multiplier to the time vector (e.g., to convert units).
...
* `scale2`: Numeric factor to multiply the `time` values (e.g., to convert from ms to s or vice versa)
