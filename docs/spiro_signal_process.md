...
sp = spiro_signal_process(
    ...
    volume,                   # list or 1D array of volumes
    flow,                     # list or 1D array of flows
    patientID,                # unique patient identifier
    trialID,                  # trial identifier
    flag_given_signal_is_FE,  # boolean: True if input is FE only
    scale1                    # float: scaling factor to apply to time (e.g., to convert ms to s)
)
```
...
* `scale1`: Scaling factor to apply to the `time` input (e.g., if time is in milliseconds, use `scale1=0.001`)
...
sp = spiro_signal_process(
    time, volume, flow,
    patientID='P1', trialID='T1', flag_given_signal_is_FE=False, scale1=0.001
)
