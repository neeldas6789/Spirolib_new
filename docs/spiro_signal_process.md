...
* `time`: Time array of the spirometry manoeuvre (list or 1D array, preferably in original units)
...
* `scale`: Multiplicative factor applied to the input `time` array to convert it into seconds (e.g., if time is in ms, use `scale=0.001`).
