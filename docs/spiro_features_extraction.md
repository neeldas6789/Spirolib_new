...
* `age`: age in years
* `height`: height in cm
* `race`: reference race code or descriptor (required by the constructor; not used in current calculations)
...
* `run_simulation(sim_param, num_sims, percentage_step, plot_FVL_only)`  
  * Runs sensitivity analysis by varying one model parameter (e.g., `zeta` or `omega`).
