...
* `get_excitation_phase(T1, params)`
  ...
  * Handles the early (excitation) phase of expiration according to the selected model.

* `calc_hypothesis(params)`

  * Simulates the flow–volume signal using the provided parameters.
...
* `run_model(excitation_type, plot_model=False, add_title_text="", plot_FVL_only=False)`
  ...
  * Fits the balloon model using `differential_evolution`. Supported `excitation_type` values:
    ...
    - `"Non linear"`
    - `""` or any other: default (initial conditions from PEF)
  * After optimization, stores fitted parameters (`wn`, `zeta`, and `alpha` where applicable) and computes fit metrics.

* `run_simulation(sim_param, num_sims, percentage_step, plot_FVL_only)`

  * Runs sensitivity analysis by varying one model parameter at a time and plots the effect on FE loop.
...
The `excitation_type` parameter now actively controls which submodel is used for the excitation phase. All previously available types (`Linear`, `Exponential pressure`, `Non linear`) remain supported alongside the default (PEF-based) model.
