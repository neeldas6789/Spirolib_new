...
* `FE_volume`: array of forced expiratory volumes
* `FE_flow`  : array of forced expiratory flows
* `sex`      : 1 for male, 0 for female
* `age`      : age in years
* `height`   : height in cm
* `race`     : string indicating reference population (required by API, e.g., 'Caucasian')
...
* `get_excitation_phase(T1, params)`
  ...
  * Handles early phase of expiration based on default initial conditions.
...
* `run_model(excitation_type, plot_model=False, add_title_text="", plot_FVL_only=False)`
  ...
  * Fits model using `differential_evolution` optimizer and plots results. The `excitation_type` parameter is now primarily internal; only the default behavior (initial conditions from PEF) is actively modeled.
