...
* `FE_volume`: 1D array of volume values for FE signal
* `FE_flow`: 1D array of flow values for FE signal
* `sex`: 1 (male) or 0 (female)
* `age`: in years
* `height`: in cm
* `race`: demographic category (e.g., 'Caucasian', 'African', 'Asian') used in the prediction model
