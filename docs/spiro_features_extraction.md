...
Models the FE signal using second-order ODE dynamics. Simulates the lungs as a deflating balloon, with optional excitation-phase modeling.
...
* `get_excitation_phase(T1, params)`
  ...
  * Handles early expiration phase based on `excitation_type` and parameters
...
* `run_model(excitation_type, plot_model=False, add_title_text="", plot_FVL_only=False)`
  ...
  * Fits model using `differential_evolution` optimizer.
  * Supported `excitation_type` options:
    - `"Linear"`
...
The `run_model` method supports multiple `excitation_type` options that influence how the expiration excitation phase is modeled:
...
These options determine the computed excitation phase and subsequent deflation dynamics.
