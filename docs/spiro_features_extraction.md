...
* `get_excitation_phase(T1, params)`
  ...
  * Internally handles the early phase of expiration (excitation) based on the method specified by `excitation_type`.
...
* `run_model(excitation_type, plot_model=False, add_title_text="", plot_FVL_only=False)`
  ...
  * Fits model using `differential_evolution` optimizer and plots results. The `excitation_type` argument supports `'Linear'`, `'Exponential pressure'`, `'Non linear'`, or default initial conditions at PEF.

* `run_simulation(sim_param, sim_type="", num_sims=4, percentage_step=10, plot_FVL_only=True)`

  * Runs sensitivity analysis by varying one model parameter (`sim_param` can be `'zeta'`, `'omega'`, or `'alpha'`). `sim_type` specifies which excitation model to simulate.
...
The `excitation_type` parameter controls the excitation-phase model:
...
Each option drives a different hypothesis for the early FE phase before the balloon deflation stage.
...
db.run_simulation(sim_param='zeta', sim_type='Non linear', num_sims=5, percentage_step=20)
```
