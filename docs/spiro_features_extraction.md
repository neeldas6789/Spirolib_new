...
* `get_excitation_phase(T1, params)`
  * Handles the early phase of expiration based on chosen excitation type or default initial conditions.
...
* `run_model(excitation_type, plot_model=False, add_title_text="", plot_FVL_only=False)`
  * Fits model using `differential_evolution` optimizer. Available `excitation_type` options:
    * `"Linear"` – linear increase to PEF
    * `"Exponential pressure"`
    * `"Non linear"`
    * `""` (default) – uses initial conditions at PEF for deflation phase
  * Set `plot_model=True` to display model overlay.

* `run_simulation(sim_param, sim_type, num_sims=4, percentage_step=10, plot_FVL_only=True)`
  * Runs sensitivity analysis by varying one model parameter (`sim_param`) over `num_sims` steps.
  * `sim_type` controls which simulation variant to use (e.g., `"Exponential pressure"`).
...
db.run_model(excitation_type="", plot_model=True)  # specify type for excitation phase
```
