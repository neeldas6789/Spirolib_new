...
```python
# include race parameter in signature
...
* `get_excitation_phase(T1, params)`  
  * Handles the early phase of expiration (excitation) based on chosen excitation type.
...
* `run_model(excitation_type, plot_model=False, add_title_text="", plot_FVL_only=False)`  
  * Fits model using `differential_evolution` optimizer. Supported `excitation_type` options: `'Linear'`, `'Exponential pressure'`, `'Non linear'`, and default (uses initial conditions at PEF).
...
* `run_simulation(sim_param, sim_type='', num_sims=4, percentage_step=10, plot_FVL_only=True)`  
  ...
  * `sim_param` chooses which parameter to vary (`'zeta'`, `'omega'`, or `'alpha'`).  
  * `sim_type` specifies excitation type for the simulation (`''`, `'Exponential pressure'`, `'Non linear'`).
...
* `'Non linear'`
* Default (uses PEF-based initial conditions)

Based on the chosen type, the deflating balloon model is fitted accordingly.
