...
* `get_excitation_phase(T1, params)`  
  Internally handles the early phase of expiration (excitation) based on default or specified model parameters.
...
* `run_model(excitation_type, plot_model=False, add_title_text="", plot_FVL_only=False)`  
  Fits the specified `excitation_type` using `scipy.optimize.differential_evolution` and stores model parameters. Supported `excitation_type` values:
  - `"Linear"`
  - `"Exponential pressure"`
  - `"Non linear"`
  - default (initial conditions at PEF)
  Set `plot_model=True` to visualize comparison.

* `run_simulation(sim_param, num_sims, percentage_step, plot_FVL_only)`  
  Runs sensitivity analysis by varying one model parameter while keeping the chosen `excitation_type`.
...
# Fit balloon model using default PEF-initialized deflation
