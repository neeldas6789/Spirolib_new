...
* `calc_AC(plotModel=False, plotProcess=False)`
  * Returns computed angle of collapse and the minimal error. Set `plotModel=True` to plot the fitted model, and `plotProcess=True` to visualize the fitting process.
...
* `get_excitation_phase(T1, params)`
  * Handles the early phase of expiration (excitation) based on model parameters.
...
* `run_model(excitation_type, plot_model=False, add_title_text="", plot_FVL_only=False)`
  * Fits the chosen model using `scipy.optimize.differential_evolution`.
  * Supported `excitation_type` options: `'Linear'`, `'Exponential pressure'`, `'Non linear'`, or default (initial‐condition model at PEF).
  * Use `plot_model` to display result, `add_title_text` to prepend to plot title, and `plot_FVL_only` to restrict to flow-volume loop.
