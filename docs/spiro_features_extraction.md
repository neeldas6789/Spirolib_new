...
* `get_excitation_phase(T1, params)`
  ...
  * Handles the early phase of expiration (excitation) based on provided parameters
...
The `run_model` method supports multiple `excitation_type` options to define the initial forcing phase:

* `Linear` – Linear excitation model.
* `Exponential pressure` – Exponential pressure excitation.
* `Non linear` – Nonlinear excitation model.
* `Default` (empty string or other) – Uses volume and flow at PEF as initial conditions for the deflation phase.

Pass the `excitation_type` string to `run_model` to select the desired excitation behavior.
...
db.run_model(excitation_type="Default", plot_model=True)  # select desired excitation type
```
